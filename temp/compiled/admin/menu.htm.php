...
<li class=menu>
<ul>
<?php $_from = $this->_var['menus']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('k', 'menu');$this->_foreach['menu'] = array('total' => count($_from), 'iteration' => 0);
...
<?php else: ?>
  ...
  <A  href="javascript:void(0);" <?php if ($this->_foreach['menu']['iteration'] == 12): ?>style="color:#FA841E"<?php endif; ?>><em  id="h1_<?php echo $this->_var['k']; ?>"></em><?php echo $this->_var['menu']['label']; ?><SPAN id="fa_<?php echo $this->_var['k']; ?>"></SPAN></A></li>
  <li  class="dropdown" id="<?php echo $this->_var['k']; ?>">
     <?php if ($this->_var['menu']['children']): ?>
    <ul>
    <?php $_from = $this->_var['menu']['children']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'child');$this->_foreach['child'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['child']['total'] > 0):
    foreach ($_from AS $this->_var['child']):
        $this->_foreach['child']['iteration']++;
?>
      <li><a href="<?php echo $this->_var['child']['action']; ?>" <?php if ($this->_foreach['menu']['iteration'] == 12): ?>target="_blank"<?php else: ?>target="main_frame"<?php endif; ?>><?php echo $this->_var['child']['label']; ?></a></li>
