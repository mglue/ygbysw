...
$json   = new JSON;
$result = array('error' => 0, 'message' => '', 'content' => '', 'count' => array());
    ...
    else
    {
        if ((intval($_CFG['captcha']) & CAPTCHA_COMMENT) && gd_version() > 0)
        {
            ...
            else
            {
                ...
                {
                    add_comment($cmt);
					$sql="select * from ".$ecs->table('comment')." where comment_type=0 and status=1 and comment_rank!=0 and id_value=".$cmt->id;
					...
					if($coun12t>0){
						...
						$count['d'] = $sixing; 
						$count['e'] = $wuxing;
						$count['lv'] = round($haoping/($coun12t*5)*100,1);
					}
					$result['count'] = $count;
                }
            }
